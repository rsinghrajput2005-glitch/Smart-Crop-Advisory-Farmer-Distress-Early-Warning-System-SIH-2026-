...
app = FastAPI(
    ...
    description=(
        ...
        "advisory and early farmer-distress risk detection. — SIH 2026"
    ),
    version="1.0.0",
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allowing all for hackathon prototype
...
@app.get("/", tags=["Health"])
def root():
    """API health check and endpoint index."""
    return {
        "status": "ok",
        "project": "Smart Crop Advisory & Farmer Distress Early-Warning System",
        "version": "1.0.0",
        "docs": "/docs",
        "endpoints": {
            ...
            "soil": "GET /soil?lat=&lon=",
            "ndvi": "GET /ndvi?lat=&lon=",
            "mandi": "GET /mandi/prices?commodity=&state=",
            "chat": "POST /chat",
            "auth": "/auth",
            "farms": "/farms",
        }
...
def advisory_endpoint(req: CropAdvisoryRequest):
    """
    Generate AI-powered crop advisory based on soil, weather, NDVI, and crop data.
    Falls back to rule-based advisory if ML models are not yet trained.
    """
    try:
        ...
        soil_ph = soil.get("ph", 7.0) or 7.0
        organic_carbon = soil.get("organic_carbon", 10.0) or 10.0
        clay = soil.get("clay", 20.0) or 20.0
        sand = soil.get("sand", 40.0) or 40.0
        
        rainfall_mm = sum([d.get("rainfall_mm", 0.0) for d in weather.get("forecast", [])])
        temperature_c = weather.get("current", {}).get("temperature_c", 25.0) or 25.0
        humidity_pct = weather.get("current", {}).get("humidity_pct", 60.0) or 60.0
...
def risk_endpoint(req: DistressRiskRequest):
    """
    Predict farmer distress risk level (Low/Medium/High) combining NDVI,
    weather, and mandi price signals. High risk triggers officer dashboard alert.
    """
    try:
        weather = get_weather_data(req.lat, req.lon)
        ndvi_data = get_ndvi(req.lat, req.lon)
        ...
        mandi = get_mandi_prices(commodity, location="Odisha")
